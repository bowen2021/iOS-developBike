...
@implementation XSDashboardProgressLayer
...
- (instancetype)initWithLayer:(id)layer{
    if (self = [super initWithLayer:layer]) {
        if ([layer isKindOfClass:[XSDashboardProgressLayer class]]) {
 ...
 */
+(id)layer{
    XSDashboardProgressLayer *layer = [[XSDashboardProgressLayer alloc]init];
    return layer;
}
...
- (void)drawInContext:(CGContextRef)ctx{
    if (_autoAdjustAngle == YES) {
        ...
        _beginAngle = k2Pi * 0.255 + totalAngle * 0.5;
    }
    // 根据中点绘制层
